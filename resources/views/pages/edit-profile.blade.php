...
<div class="login-box">
  ...
  {{-- Alert message if error or success --}}
@if(session('error'))
<div id="error-msg" class="login-err" style="display:block;background:#f8d7da;color:#721c24;padding:0.8rem 1rem;border-radius:8px;border:1.5px solid #f5c6cb;margin-bottom:1rem;word-break:break-word;white-space:normal;">❌ {{ session('error') }}</div>
@endif

@if($errors->any())
<div id="errors-msg" class="login-err" style="display:block;background:#f8d7da;color:#721c24;padding:0.8rem 1rem;border-radius:8px;border:1.5px solid #f5c6cb;margin-bottom:1rem;word-break:break-word;white-space:normal;">❌ {{ $errors->first() }}</div>
@endif
  ...
  <form method="POST" action="/profile/edit">
    ...
    @if($field == 'name')
      <input type="text" name="value" class="login-field"
             ...
             value="{{ old('value', $user->name) }}" required>

    @elseif($field == 'email')
      <input type="email" name="value" class="login-field"
             placeholder="📧 Email"
             value="{{ old('value', $user->email) }}" required>
    ...
    @elseif($field == 'phone')
      <input type="text" name="value" class="login-field"
             ...
             value="{{ old('value', $user->phone) }}" required>
