...
<div style="background:linear-gradient(135deg,#0d2b5e 0%,#163a7a 100%);border:3px solid #1A1A1A;border-radius:18px;box-shadow:6px 6px 0 #1A1A1A;padding:3rem 2rem 3rem;position:relative;display:flex;flex-direction:column;align-items:center;gap:0.9rem;text-align:center;">
  ...
  <div style="font-family:'Boogaloo',cursive;font-size:4.5rem;color:#ffffff;font-weight:bold;line-height:1.1;">{{ $user->name }}</div>
  <div style="font-size:1rem;color:rgba(255,255,255,0.55);font-weight:600;margin-top:-0.3rem;">{{ $user->email }}</div>
</div>

{{-- Personal Info --}}
  <div>
    <div class="section-title">{{ __('messages.personal_info') }}</div>
    <div class="info-card">
      <div class="info-row">
        <div class="info-ic">👤</div>
        <div><div class="info-label">{{ __('messages.full_name') }}</div><div class="info-value">{{ $user->name }}</div></div>
        <a href="/profile/edit?field=name"  class="info-edit">{{ __('messages.edit') }}</a>
      </div>
      <div class="info-row">
        <div class="info-ic">📧</div>
        <div><div class="info-label">{{ __('messages.email') }}</div><div class="info-value">{{ $user->email }}</div></div>
        <a href="/profile/edit?field=email"  class="info-edit">{{ __('messages.edit') }}</a>
      </div>
      <div class="info-row">
        <div class="info-ic">🎂</div>
        <div><div class="info-label">Password</div><div class="info-value">••••••••</div></div>
        <a href="/profile/password" class="info-edit">{{ __('messages.edit') }}</a>
      </div>
      <div class="info-row">
        <div class="info-ic">📱</div>
        <div><div class="info-label">{{ __('messages.phone') }}</div><div class="info-value">
        {{ substr($user->phone, 0, 3) . '-' . substr($user->phone, 3) }}</div></div> 
        <a href="/profile/edit?field=phone"  class="info-edit">{{ __('messages.edit') }}</a>
      </div>
      <div class="info-row">
        <div class="info-ic">🎂</div>
        <div><div class="info-label">{{ __('messages.birthday') }}</div><div class="info-value">{{ $user->birthday }}</div></div>
      </div>
      <div class="info-row">
        <div class="info-ic">📅</div>
        <div><div class="info-label">{{ __('messages.member_since') }}</div><div class="info-value">{{ \Carbon\Carbon::parse($user->created_at)->format('F Y') }}</div></div>
      </div>
    </div>
  </div>
...
{{-- Logout --}}
<a href="{{ route('logout') }}" style="background:var(--r);border:3px solid var(--blk);border-radius:var(--radius);box-shadow:var(--sh);padding:1.25rem 1.5rem;display:flex;align-items:center;gap:1rem;cursor:pointer;transition:all 0.12s;text-decoration:none;">
...
<div class="stats-row">
  <div class="stat-card">
    ...
    <div class="stat-key">{{ __('messages.total_visits') }}</div>
  </div>
